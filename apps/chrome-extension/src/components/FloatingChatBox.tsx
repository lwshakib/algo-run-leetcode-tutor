...
export default function FloatingChatBox({
  ...
  const sendMessage = async () => {
    ...
    // 5. Retrieve and decrypt the Gemini API key and get the selected model from storage
    const storage = await chrome.storage.local.get([
      "gemini_api_key",
      "gemini_model",
    ])
    const encryptedKey = storage.gemini_api_key
    const selectedModel = (storage.gemini_model as string) || "gemini-2.0-flash"
    ...
    const result = streamText({
      model: getGoogleModel(apiKey, selectedModel),
      ...
      <div className="chat-header">
        <div className="header-left">
          <div className="logo-container">
            <img
              src={chrome.runtime.getURL("logos/logo.svg")}
              alt="Algorun Logo"
              className="logo-img"
            />
